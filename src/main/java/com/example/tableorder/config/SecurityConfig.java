...
/**
 * Spring Security 설정 클래스. - 모든 /api/** 엔드포인트에 JWT 인증 적용 (OWNER 역할 필수). - 세션 사용
 * 안 함 (STATELESS). - JWT 필터 추가. - Swagger UI 및 OpenAPI 엔드포인트 (/v3/api-docs/**
 * 등) permitAll (테스트용, 프로덕션에서 제거 추천).
 */
@Configuration
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .anyRequest().authenticated() // 기타는 인증 필요
                )
